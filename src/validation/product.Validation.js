...
const productSchema = {
  createProduct: {
    body: Joi.object({
      sku_id: Joi.number().integer().positive().required().messages({
      ...
      cost_price: Joi.number().positive().precision(2).required().messages({
        'number.base': 'Cost price must be a number.',
        'number.positive': 'Cost price must be a positive number.',
        'any.required': 'Cost price is required.',
      }),

      gst_percent: Joi.number().min(0).max(100).precision(2).required().messages({
      ...
      packaging_type: Joi.string().max(100).optional().messages({
      ...
      images: Joi.array()
        .items(
          Joi.string().uri().max(500).messages({
  ...
  updateProduct: {
    ...
    body: Joi.object({
      approved_by: Joi.number().integer().positive().optional().messages({
      ...
      sku_id: Joi.number().integer().positive().optional().messages({
        'number.base': 'SKU ID must be a number.',
        'number.integer': 'SKU ID must be an integer.',
        'number.positive': 'SKU ID must be a positive number.',
      }),

      product_name: Joi.string().min(2).max(255).optional().messages({
        'string.base': 'Product name must be a string.',
        'string.min': 'Product name must be at least 2 characters long.',
        'string.max': 'Product name cannot exceed 255 characters.',
      }),

      mrp: Joi.number().positive().precision(2).optional().messages({
        'number.base': 'MRP must be a number.',
        'number.positive': 'MRP must be a positive number.',
      }),

      cost_price: Joi.number().positive().precision(2).optional().messages({
        'number.base': 'Cost price must be a number.',
        'number.positive': 'Cost price must be a positive number.',
      }),

      gst_percent: Joi.number().min(0).max(100).precision(2).optional().messages({
        'number.base': 'GST percent must be a number.',
        'number.min': 'GST percent cannot be negative.',
        'number.max': 'GST percent cannot exceed 100.',
      }),
      ...
      approval_status: Joi.string().valid('P', 'A', 'R').optional().messages({
